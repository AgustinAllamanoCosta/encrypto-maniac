...
class ConsultaDB:
	...
	ingresarUsuario = 'INSERT INTO usuarios(usuario,contrasenia,contraseniaRecupero,rutaArchivoKey) VALUES (?,?,?,?)'
	buscarArchivo = 'SELECT rutaArchivoKey FROM usuarios WHERE usuario == ?'
	crearTablaClaves = '''CREATE TABLE clavesYAplicaciones ( codigo integer PRIMARY KEY autoincrement,nombreApp text,clave text);'''
	crearTablaUsuario = '''CREATE TABLE usuarios ( codigo integer PRIMARY KEY autoincrement,usuario text,contrasenia text, contraseniaRecupero text, rutaArchivoKey text);'''